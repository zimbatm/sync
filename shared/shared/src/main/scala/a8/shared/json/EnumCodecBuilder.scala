...
object EnumCodecBuilder {
  ...
  def apply[A <: enumeratum.values.StringEnumEntry](enum0: enumeratum.values.StringEnum[A]): JsonCodec[A] =
    new JsonCodec[A] {
  ...
  def apply[A <: enumeratum.EnumEntry](enum0: enumeratum.Enum[A]): JsonCodec[A] =
    new JsonCodec[A] {
